...
├── RUNBOOK_L1SW_TO_P6.md                        ← ★ 수행 순서 설명서 (L1SW 연동→P0~P6, 손동작 중심)
├── reference/
...
│       ├── RCA_ANALYSIS_METHODOLOGY.md          ← ★★ 원인분석 7단계 방법론 (P4 참조, 성공률/효율 핵심)
...
## 10. 2026-06-23 적용 사항 — 정합성 수정 + 원인분석 방법론 강화

이번 세션에서 두 가지를 적용했다.

### 10.1 signature 네임스페이스 정합성 통일 (1번)

P4 규칙은 `signature_set`에 keywords.yaml signature ID 사용을 요구하나, 예시 파일들이
alias(`Msg1`, `beamFail` 등)를 쓰고 있어 모순이었다. ID 표기로 통일했다.

수정 파일:
- `rca_kg/schema/rca_case.schema.yaml` (fingerprint example → `RACH_MSG1...` ID)
- `rca_kg/cases/EXAMPLE_v2_rach_failure_001.yaml` (signature_set/sequence/log_pattern)
- `rca_kg/cases/unresolved/EXAMPLE_unresolved.yaml` (`SCG_CONFIG`/`SCG_BEAM_FAIL`)

검증: 3개 파일 YAML 파싱 OK, 모든 signature 참조가 keywords.yaml ID로 resolve됨.

### 10.2 deepdive 폴더 정리

- `L1SW_AND_FLOW_20260620.md`: 깨진 근거 파일명(`..._20260620.md` → `..._v2.md`) 수정
- `L1SW_VERIFICATION_QUESTIONS_v2.md`, `L1SW_CHECKLIST_OBJECTIVE_v2.md`: 상단에
  "✅ 사용 완료, 결과는 AND_FLOW에 반영, 빈 양식은 이력 보존용" 마커 추가
  (다음 세션이 미완성 양식으로 오인하지 않도록)

### 10.3 원인분석 방법론 신설 — 성공률·효율 강화

기존 자료는 워크플로(root_cause를 어디 쓰는지)는 정의했으나 "L1SW 출력에서 어떻게 원인을
찾는가" 하는 추론 방법이 비어 있었다. 이를 채웠다.

신규 파일:
- `prompt/deepdive/RCA_ANALYSIS_METHODOLOGY.md` — issue_type 공통 7단계 추론 엔진
  (증상 anchor → 시간창 → 정상경로 대조로 최초 이탈 특정 → 가설 2~4개 →
   가르는 단서만 grep → 단말/환경 분리 → 인과사슬+confidence)
  + confidence 산정 기준(증거 충족도), 효율 규칙, 안티패턴, case 필드 매핑 포함.

수정 파일:
- `prompt/CLAUDE_CODE_PROMPTS_e2e_v1.md`
  - P4: 방법론 문서 첨부 + PART A(7단계 추론)/PART B(YAML 매핑) 2부 구조로 재편.
        root_cause.summary는 인과사슬로, confidence는 §3 표대로 강제.
  - P5: 자가점검에 ID 표기/인과사슬 품질/confidence 정합/도메인 분리 항목 추가.

핵심 설계 의도:
- **성공률**: 첫 가설 확증·증상을 원인으로 적기 등 안티패턴을 명시 차단. 최소 2개 가설
  강제, confidence를 증거 충족도에 묶어 과신 방지.
- **효율**: 전체 정독 금지. anchor부터 바깥으로, 가설이 부르는 grep만 수행. cptime 단조성
  활용. 3회 규칙으로 무한 정독 차단.
- skills_seed(issue_type별 단서 표)는 이 공통 엔진의 S3~S5에 꽂아 쓰는 부품으로 위치 정리.

다음 세션 권장: 사내 P0→P1→P2(R4)에 이어 실로그 1건으로 P3→P4(방법론 적용)→P5 E2E를
돌려 "방법론이 실제 root_cause 정확도를 올리는지" 검증.
---

## 11. 2026-06-23 추가 — 수행 순서 설명서 신설

사용자 요청에 따라 L1SW 스킬 연동부터 시작하는 단계별 실행 설명서를 추가했다.

신규 파일:
- `RUNBOOK_L1SW_TO_P6.md` — STEP A(L1SW 스킬 연동 확인) → B(P0) → C(P1) → D(P2) →
  E(L1SW 실행) → F(P3) → G(P4 원인분석) → H(P5 승인) → I(P6 승격) 순서를
  "무엇을 하나 / 사람이 보는 것 / 손동작"으로 정리. 최초 1회 셋업(A~D)과 로그마다
  반복(E~I)을 구분. 막혔을 때 분기표 포함.

수정 파일:
- `USAGE_SCENARIO.md` — 상단에 RUNBOOK 링크 추가(순서대로 따라 하려면 RUNBOOK)
- `HANDOFF.md` — 트리 + 본 섹션

문서 역할 분담 정리:
- USAGE_SCENARIO.md = "무엇을 왜" (시나리오 3종, 개념)
- RUNBOOK_L1SW_TO_P6.md = "순서와 손동작" (L1SW 연동부터 P6까지)
- CLAUDE_CODE_PROMPTS_e2e_v1.md = 프롬프트 본문 (P0~P6)
- RCA_ANALYSIS_METHODOLOGY.md = 원인분석 추론법 (P4 PART A)
- scripts/README.md = 수동형(모드 B) 상세

parse.ps1 입력 파라미터명은 환경 의존이라 RUNBOOK에서 임의로 정하지 않고
P0/P1 probe 결과를 쓰도록 명시(추측 금지 원칙 유지).
