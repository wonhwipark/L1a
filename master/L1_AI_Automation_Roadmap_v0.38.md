# L1 AI Automation Roadmap v0.38
---
## Document Purpose
이 문서는 L1 AI Automation Roadmap의 v0.38 Master Document이다.
...
---
### 5.3.0 Confluence Child Page Discovery Strategy
```text
Parent Page Link 입력
  ↓
Discovery Method 순차 시도
  (REST API → MCP → 본문 링크 추출 → Label → Title → Fuzzy → User Assisted)
  ↓
성공한 Method 기록
  ↓
Discovery Strategy Profile 저장
  (%USERPROFILE%\artifacts\child_page_discovery_profile.json)
```
Confluence Child Page Discovery Strategy는 5.3 Weekly Report Collection의 선행 의존성 항목이다.
주간보고 Parent Page는 매주 새로 생성되므로, 특정 주차의 Child Page URL이나 ID가 아니라 Child Page를 찾는 방식(method)만 저장하여 주차 간 재사용 가능하도록 한다.
Profile에는 성공한 discovery method, confidence, fallback method list만 포함하며, 특정 주차 데이터는 포함하지 않는다.
**Expected Benefit**
- 매주 반복되는 Child Page 탐색 방식 검증 부담 제거; discovery method 재사용으로 5.3 실행 안정성 향상; 탐색 실패 시 fallback 경로 사전 확보
**Precondition**
- Confluence MCP + REST API 접근 가능; Parent Page Link를 사용자가 매주 제공
**Storage / Reuse**
- Discovery Strategy Profile(`%USERPROFILE%\artifacts\child_page_discovery_profile.json`); 저장 대상: 성공 method, confidence, fallback list; 저장 제외: 특정 주차 Parent Page ID/URL, Child Page ID/URL 목록; 5.3 Weekly Report Collection이 매주 이 profile을 참조
---
